...
\begin{align}
  & q_{hk}^x = q_{k}^x = \frac{2\pi k}{\lambda_r} \\
  & q^y = 0 \\
...
position, $(X,Z)$, corresponds to a triplet $(q^x, q^y, q^z)$. Fully exploring 
...
\begin{align}
  \qx{} &\approx \frac{4\pi\theta\cos\phi}{\lambda} \approx kX/S \nonumber\\
  \qy{} &\approx q_z\omega -\frac{4\pi\theta^2}{\lambda} \approx q_z\omega - \frac{\lambda q_z^2}{4\pi}\nonumber\\
  \qz{} &\approx \frac{4\pi\theta\sin\phi}{\lambda} \approx kZ/S,
...
$\qz{}$ is constant at a given pixel as $\omega$ is changed, which can be seen from 
Eq.~(\ref{eq:qxqyqz_small}). $\omega$ dependence appears only through $\qy{}$, 
so rotating the sample is realized by integrating over $\qy{}$. 
...
is $2\times \lambda \qz{}/(4\pi)$ for scattering with $\qz{}$. 
Thus, the integration limits 
for $\qy{}$ integration are $[-\lambda \qz{}^2/(4\pi), \lambda \qz{}^2/(4\pi)]$.
...
\begin{align}
  \Io{hk} 
    ...
    &\propto |F_{hk}|^2 \int dq_x \int dq_z 
             \int_{-\frac{\lambda \qz{}^2}{4\pi}}^{\frac{\lambda \qz{}^2}{4\pi}} \frac{d\qy{}}{\qz{}} 
             S_{hk}(\mathbf{q}),
\end{align}
